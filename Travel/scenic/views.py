...
def add_s2(request):
    ...
    for line in db2:
        list2 = line.split('#')
        if len(list2) ==12:
            scen2 = models.Scbr(id=list2[0], sce_name1=list2[1],grage=list2[2], sce_addr=list2[3], open_time=list2[4],
...
def add_tic(request):
    ...
    for line in db3:
        list3 = line.split('#')
        if len(list3) ==4:
           tic = models.Ticket(id=list3[0],type=list3[1],name=list3[2],price=list3[3])
           tic.save()
    db3.close()
    return HttpResponse('插入成功')
